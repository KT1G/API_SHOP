    ...
    try {
        ...
        if (idArray) {
            ...
            if (notExist.length > 0) {
                ...
                throw generateError(
                    `Not Found. Los productos con id: ${notExist} no existen, se borraron o se compraron, debe ser un numero entero entre 1 y ${maxId[0].maxId}`,
                    404
                )
                ...
                throw generateError(
                    `Forbidden. Los productos con id: ${notDelete} no se pueden borrar porque no eres el dueño ni tienes permisos de administrador`,
                    403
                )
            } else {
            ...
            if (product.length === 0) {
                throw generateError(
                    `Not found. El producto con id: ${id} no existe`,
                    404
                )
            } else if (adminProductUser[0].status === 'admin') {
                throw generateError(
                ...
                //Calcular los likes del dueño del producto
                const [likes] = await connection.query(
                    `SELECT COUNT(id) AS likes FROM products WHERE user_id=${userIdProduct}`
                )
                //Actualizar los likes del dueño del producto
                await connection.query(
